...
import MentorSkeleton from "../MentorSkeleton/MentorSkeleton";
...
const Mentor = ({ data, isLoading }: MentorProps) => {
    ...
    <div className={styles.mentorContainer}>
      {isLoading
        ? Array.from({ length: 6 }).map((_, index) => (
            <div key={index} className={styles.mentorContainerItem}>
              <MentorSkeleton />
            </div>
          ))
        : data?.map((el) => (
            <div key={el.user.email} className={styles.mentorContainerItem} data-testid="mentor">
              <div className={styles.mentorItemProfile}>
                ...
                <img className={styles.mentorProfileImage} src={el.user.image[0].fileUrl} />
                <button
